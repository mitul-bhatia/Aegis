...
## Aegis wiring (this repo)

| Piece | Path |
|-------|------|
| Task / done criteria | `RALPH_TASK.md` |
| Progress / signs | `.ralph/progress.md`, `.ralph/guardrails.md` |
| CLI scripts | `.cursor/ralph-scripts/` + wrapper `./scripts/ralph` |
| In-IDE agent | `.cursor/agents/ralph-loop.md` (`/ralph-loop`) |
| Rule | `.cursor/rules/ralph-loop.mdc` |
| Coding discipline | `.cursor/rules/karpathy-guidelines.mdc` + `engineering-discipline.mdc` |
| Project brain | `docs/PROJECT_MEMORY.md` |

**Models:** Interactive Agent can use Grok 4.5 now. Overnight CLI defaults to `composer-2` via `RALPH_DEFAULT_MODEL` / override with `RALPH_MODEL=...`. Requires `cursor-agent` for `./scripts/ralph loop`.

**One iteration (IDE):** Read task + progress + guardrails → complete next `[ ]` → verify → update files → emit `<ralph>CONTINUE\|COMPLETE\|GUTTER</ralph>`.
