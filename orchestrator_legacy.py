...
from utils.crypto import decrypt_token  # decrypt token before using it for git/GitHub API
...
def _create_scan(db, repo_id, commit_sha: str, branch: str) -> Scan:
    """
    Create a new scan record, or return the existing one if this
    commit was already scanned (prevents duplicate runs from GitHub
    webhook retries sending the same payload twice).
    """
    # Check if a scan already exists for this exact commit + repo
    ...
    if existing:
        logger.info(
            f"Duplicate scan detected for commit {commit_sha[:8]} "
...
def run_aegis_pipeline(push_info: dict):
    ...
    try:
        ...
        if repo_id:
            ...
            # If the scan already reached a terminal state (fixed, clean, failed, etc.)
            # it means this is a duplicate webhook — skip the pipeline entirely.
            terminal_statuses = {
            ...
            if scan.status in terminal_statuses:
                logger.info(
                    f"Scan #{scan.id} already completed with status '{scan.status}' "
                    f"— ignoring duplicate webhook for {commit_sha[:8]}."
                )
                return
        ...
        if repo_obj and repo_obj.user:
            # Decrypt the stored token before using it for git clone / GitHub API calls
        ...
        # ── Phase 5: Agent 2 (Exploiter) — Prove ALL vulnerabilities ────────────────────
        # We test every finding and collect ALL confirmed ones (no early break).
        # This means a commit with 3 real vulns gets 3 PRs, not just 1.
        confirmed_vulnerabilities = []

        for i, finding in enumerate(findings, 1):
            logger.info(
                f"🎯 Agent 2 (Exploiter): Testing {i}/{len(findings)}: {finding.vuln_type}"
            )
            ...
            if scan:
                update_scan_status(scan.id, ScanStatus.EXPLOITING.value, {
                    ...
                    "agent_message": (
                        f"Testing vulnerability {i}/{len(findings)}: "
                        f"{finding.vuln_type} in {finding.file}:{finding.line_start}..."
                    ),
                })

            # Convert Pydantic model to plain dict for the exploiter
            finding_dict = {
                "file": finding.file,
            ...
            if scan:
                update_scan_status(scan.id, ScanStatus.EXPLOITING.value, {
                    "current_agent": "exploiter",
                    "agent_message": "Running exploit in isolated Docker sandbox...",
                    "exploit_script": exploit_script,
                })

            exploit_test = run_exploit_in_sandbox(exploit_script, local_repo_path)

            # Hard stop if Docker is unavailable — unsafe to continue
            if "SANDBOX_UNAVAILABLE" in exploit_test.get("stderr", ""):
                logger.error("Docker sandbox unavailable — aborting scan for security.")
                if scan:
            ...
            if exploit_test["exploit_succeeded"]:
                ...
                if scan:
                    update_scan_status(scan.id, ScanStatus.EXPLOIT_CONFIRMED.value, {
                        "current_agent": "exploiter",
                        "agent_message": (
                            f"Confirmed {i}/{len(findings)}: "
                            f"{vulnerability_type} is exploitable"
                        ),
                        "exploit_output": exploit_test["stdout"],
                    })
            else:
        ...
        if not confirmed_vulnerabilities:
            ...
            if scan:
                update_scan_status(scan.id, ScanStatus.FALSE_POSITIVE.value, {
                    "current_agent": None,
                    "agent_message": "All exploits failed — marked as false positive",
                    "exploit_output": "All exploits failed to confirm vulnerabilities",
                })
            return

        # ── Phase 6+7+8: Fix ALL confirmed vulnerabilities sequentially ───────────────
        # Each confirmed vuln gets its own Engineer→Verifier loop and its own PR.
        total = len(confirmed_vulnerabilities)
        pr_urls = []

        for vuln_idx, confirmed in enumerate(confirmed_vulnerabilities, 1):
            vuln_type    = confirmed["vulnerability_type"]
            vuln_file    = confirmed["finding"]["file"]
            vuln_exploit = confirmed["exploit_script"]
            vuln_output  = confirmed["exploit_output"]

            logger.info(
                f"🔧 Fixing vulnerability {vuln_idx}/{total}: {vuln_type} in {vuln_file}"
            )

            full_vulnerable_path = os.path.join(local_repo_path, vuln_file)
            ...
            if scan:
                update_scan_status(scan.id, ScanStatus.PATCHING.value, {
                    ...
                    "agent_message": (
                        f"Patching {vuln_idx}/{total}: {vuln_type} in {vuln_file}..."
                    ),
            ...
            remediation = run_remediation_loop(
                ...
                file_path=vuln_file,
                exploit_script=vuln_exploit,
                exploit_output=vuln_output,
                vulnerability_type=vuln_type,
            ...
            if not remediation["success"]:
                logger.error(
                    f"❌ Could not patch {vuln_type} after max retries — skipping."
                )
                if scan:
                    update_scan_status(scan.id, ScanStatus.FAILED.value, {
                        "current_agent": None,
                        "agent_message": (
                            f"Failed to patch {vuln_type} after "
                            f"{remediation.get('attempts', 3)} attempts — human review needed"
                        ),
                        "error_message": f"Could not patch {vuln_type}",
                        "patch_attempts": remediation.get("attempts", 3),
                    })
                # Continue to next vulnerability even if this one failed
                continue

            # Patch verified — open a PR for this vulnerability
            if scan:
                update_scan_status(scan.id, ScanStatus.VERIFYING.value, {
                    "current_agent": "verifier",
                    "agent_message": (
                        f"Patch {vuln_idx}/{total} verified — opening PR for {vuln_type}..."
                    ),
            ...
            try:
                ...
                logger.error(f"PR creation failed for {vuln_type}: {pr_err}")
                pr_url = None
            ...
            if scan:
                update_scan_status(scan.id, ScanStatus.FIXED.value, {
                    "current_agent": None,
                    "agent_message": (
                        f"Fixed {vuln_idx}/{total}: {vuln_type}. "
        ...
        if pr_urls:
            logger.info(f"🎉 Done! {len(pr_urls)}/{total} vulnerabilities fixed. PRs: {pr_urls}")
        else:
            logger.warning("Pipeline completed but no PRs were created.")
